...
| `src/` layout, when a feature gets `domain/` or `infrastructure/`, module wiring, the `@/*` alias, build targets | `refs/api-architecture.md` |
...
Repo-wide subjects — env vars, the two databases, CI, `@packages/contracts`, **fin applet
naming** — are indexed in [`../../AGENTS.md`](../../AGENTS.md). Financial domain work must
follow [`../../refs/monorepo-fin-applet.md`](../../refs/monorepo-fin-applet.md)
(`features/fin/`, `/v1/fin/…`, `fin_*` tables).
...
- **Every Postgres identifier is snake_case** via `@map` / `@@map`. A missing map fails the test
  suite, in CI too.
- **No Prisma model and no domain feature without a product/spec decision.** `health` is the only
  feature.
...
- **New feature under `src/features/<name>/` that carries rules of its own → new ref**
  `refs/api-<name>.md`, plus a row in the routing table above, in the same PR.
